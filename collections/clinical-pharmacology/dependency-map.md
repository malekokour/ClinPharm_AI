...
|---|---|---|---|
| Safety and permitted-source preflight | `shared/contracts/source-preflight.md` | `review-csr-pk-consistency` | every skill |
| Evidence hierarchy and source locators | `shared/contracts/evidence-hierarchy.md` | `review-csr-pk-consistency` | every skill |
| Contradiction-preserving claim ledger | `shared/contracts/contradiction-ledger.md` | `review-csr-pk-consistency` | `reconcile-cross-document-facts`, `review-ddi-evidence` |
| Output-state markers (`UNKNOWN` / `NEEDS_INPUT` / `CANNOT_ASSESS`) | `shared/contracts/output-states.md` | `review-csr-pk-consistency` | every skill |
| Human-review and prepare-only boundary | `shared/contracts/human-review.md` | `review-csr-pk-consistency` | every skill |
| PK parameter plausibility and unit checks | `shared/tools/pk_plausibility.py` | `review-csr-pk-consistency` | `verify-nca-outputs`, `review-study-conduct-pk` |
| Cross-document numeric consistency | `shared/tools/cross_document_consistency.py` | `review-csr-pk-consistency` (document mode) | `reconcile-cross-document-facts` (programme mode) |
| Ratio-statistic recomputation | `shared/tools/ratio_statistics.py` | `review-csr-pk-consistency` | `verify-nca-outputs` |

**Single-source rule.** The reconciliation engine exists only in
`shared/tools/cross_document_consistency.py`. `review-csr-pk-consistency` calls
...
plausibility rules, which live only in `shared/tools/pk_plausibility.py`.
...
|---|---|---|---|
| SAD/MAD ascending dose | `shared/modules/sad-mad.md` | released | `review-csr-pk-consistency` |
| Food effect | `shared/modules/food-effect.md` | released | `review-csr-pk-consistency` |
