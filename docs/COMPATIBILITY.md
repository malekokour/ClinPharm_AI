...
Compatibility has two separate routes:

1. **Attach-first route:** upload the DOCX or Markdown starter to an ordinary
   conversation.
2. **Agent Skill route:** install `skills/build-work-context/` in a host
   that supports the open Agent Skills format.

Do not treat success in one route as proof of the other.

| Host | Attach-first route | Agent Skill route | Current evidence |
|---|---|---|---|
...
The skill follows the open specification:
...
instruction files, and `AGENTS.md` for agent instructions. ClinPharm AI does
not generate or install those files in v0.1; users may adapt the working pack
