...
XREF = re.compile(r"(?:see|refer to|per)\s+(?:Section\s+)?(\d+(?:\.\d+){1,3})", re.I)
HEADING = re.compile(r"^\s*#*\s*(\d+(?:\.\d+){1,3})", re.M)
TABLE_DEF = re.compile(r"^\s*#*\s*Table\s+(\d+)", re.I | re.M)
TABLE_CITE = re.compile(r"\bTable\s+(\d+)", re.I)
