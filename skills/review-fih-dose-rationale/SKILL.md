...
metadata:
  ...
  version: "0.1.0"
  schema-version: "1.0"
  evidence-level: synthetic-benchmark-pending-run
...
Load the first-in-human module from `shared/modules/first-in-human.md` (vendored
...
Run T01 `mrsd_calculator` (`shared/tools/mrsd_calculator.py`; vendored as
...
The tool carries a coded default safety factor and a Km conversion table it
...
`shared/tools/cross_document_consistency.py`).
...
Use the exact tokens defined in `shared/contracts/output-states.md`:
...
here that means three of them, per `shared/contracts/human-review.md` and
...
The frontmatter records `evidence-level: synthetic-benchmark-pending-run` for
that reason, and no performance figure should be quoted for it until a run exists
with its task, model, host, date and run count stated.
...
Version 0.1.0 · owner Malek Okour · reviewed 2026-08-05 · research id S02 ·
...
`ema-fih`, or the first-in-human module in `shared/modules/first-in-human.md`.
