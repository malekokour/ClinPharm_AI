...
engine from `shared/tools/cross_document_consistency.py`:
...
`shared/tools/pk_plausibility.py`: unit consistency against the declared
...
acts, detailed in `shared/contracts/human-review.md`.
...
**No benchmark fixture has been published for this skill.** Its
`evidence-level` is `not-yet-evaluated`, and no performance claim of any kind is
made for it. Sibling skills in this collection are evaluated against synthetic
packages with expert-keyed planted defects, and this one will be held to the
same route.
