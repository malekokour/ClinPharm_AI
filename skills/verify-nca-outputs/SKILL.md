...
Study-type modules under `shared/modules/` supply the expected parameter set and
...
`shared/contracts/human-review.md`. Never assume either.
...
`shared/tools/cross_document_consistency.py` and which the released package
...
`shared/tools/pk_plausibility.py`, vendored into the released package:
...
| O4 | Human-review record with sign-off block, per `shared/contracts/human-review.md` | Markdown, unset fields visibly unset |
...
`shared/contracts/contradiction-ledger.md`. Never silently harmonise, never pick
