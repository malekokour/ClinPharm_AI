...
**Ordinary chat** — attach the starter that ships with the release under
`starter/prepare-dose-justification-evidence/` — **not yet present.** This
package is `built`, and its starter ships when it passes the evaluation gate (DOCX, or the Markdown, which some
hosts accept where DOCX is rejected) together with the section you are working
on, and follow the workflow it carries.
...
planted gaps spanning every item class.
