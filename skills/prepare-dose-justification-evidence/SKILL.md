...
- `shared/modules/oncology-dose-optimization.md` — randomised dosage comparison, anchor `fda-optimus`
- `shared/modules/dose-proportionality-accumulation.md`
- `shared/modules/renal-impairment.md` and `shared/modules/hepatic-impairment.md`
- `shared/modules/drug-drug-interaction.md`, `shared/modules/food-effect.md`
- `shared/modules/ba-be-formulation-bridging.md`
- `shared/modules/biologics-mab-pk.md`, `shared/modules/immunogenicity-ada.md`
- `shared/modules/pediatric-pk-extrapolation.md`, `shared/modules/qt-assessment.md`
...
`shared/tools/renal_staging.py` (T02). That tool classifies an eGFR into its
...
Per `shared/modules/ba-be-formulation-bridging.md`, lay out the chain from the
...
`shared/contracts/human-review.md` and must be treated as invalid.
...
a contradiction, per `shared/contracts/evidence-hierarchy.md`. Never silently
...
whole package. The skill's per-skill starter ships with the release under
`starter/prepare-dose-justification-evidence/`, which ships when this package
passes its evaluation gate. It is `built` today, so the starter is not present yet.
