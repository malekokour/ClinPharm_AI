...
description: Structures a drug-drug interaction evidence package for one compound from its own in-vitro results, clinical DDI studies, modelling reports, labels and cited literature, under the ICH M12 decision logic — perpetrator and victim roles assigned per comparison, every in-vitro signal carried to a study-conducted or study-not-conducted terminus with an auditable trigger trail, and management wording drafted for review. Use this skill when someone asks to assemble, structure, review or gap-check the DDI evidence for a drug, to check whether a clinical interaction study is triggered, or to draft interaction management language for a report, summary or label section — for example "build the DDI package for compound X" or "does the CYP3A4 in-vitro result trigger a clinical study". Do not use for programme-level development-plan routing across all study types, for deciding whether an interaction is clinically significant, for selecting or adjusting a dose, or as a substitute for a curated drug-interaction database.
license: MIT
...
Criteria come from the shared drug-drug interaction module — canonical source
`shared/modules/drug-drug-interaction.md`, vendored into the released package at
build time. It supplies the design conventions, the expected statements, and the
mechanical checks. Load it before any check runs.
...
`shared/contracts/source-preflight.md`, vendored into the released package. If
...
`shared/contracts/human-review.md`. Never assume one.
...
`shared/contracts/contradiction-ledger.md` — then the outputs below.
...
`shared/contracts/output-states.md`:
...
`shared/contracts/human-review.md`.
...
**UNVERIFIED:** at version 0.1.0 no planted-defect fixture has been evaluated for
this skill. The library's own rule is that a study-type module promotes only with
its own fixture, and this skill inherits that debt rather than hiding it. Treat
every claim about its detection performance as unevidenced until a fixture and
its results ship alongside it.
...
Version 0.1.0 · owner Malek Okour · reviewed 2026-08-05 · research id S09 ·
