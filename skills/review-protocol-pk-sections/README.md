...
yet, and the activation-separation test against `review-csr-pk-consistency` — the
residual architectural risk recorded in the collection's dependency map — has not
been run. Its evidence level is `unevaluated-pending-fixture`, and no performance
claim should be made for it until both exist.
