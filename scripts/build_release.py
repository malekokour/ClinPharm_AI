#!/usr/bin/env python3
"""Build deterministic ClinPharm AI release assets and checksums.
...
def build(output: Path) -> list[Path]:
    ...
    expected_names = {p.name for p in packages} | {
        ...
        "clinpharm-ai-workflow.gif",
        "clinpharm-ai-workflow.mp4",
    ...
    for skill in skills:
        for suffix in (".md", ".docx"):
            for candidate in (ROOT / "starter" / skill.name).glob(f"*{suffix}"):
    ...
    copies = [
        (ROOT / "starter/build-work-context/Pharma-Work-Context.docx", output / "Pharma-Work-Context.docx"),
        (ROOT / "starter/build-work-context/Pharma-Work-Context.md", output / "Pharma-Work-Context.md"),
        (
            ROOT / "docs/assets/clinpharm-ai-workflow.gif",
            output / "clinpharm-ai-workflow.gif",
        ),
        (
            ROOT / "docs/assets/clinpharm-ai-workflow.mp4",
            output / "clinpharm-ai-workflow.mp4",
        ),
    ]
    ...
    if scan_tree(output, scan_report, ignored_parts=set()):
