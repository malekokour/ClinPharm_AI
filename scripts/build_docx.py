#!/usr/bin/env python3
"""Build polished DOCX artifacts from canonical ClinPharm AI Markdown.
...
def style_document(document: Document) -> None:
    ...
    if "ClinPharm AI Quote" not in document.styles:
        quote = document.styles.add_style("ClinPharm AI Quote", WD_STYLE_TYPE.PARAGRAPH)
    ...
    if "ClinPharm AI Code" not in document.styles:
        code = document.styles.add_style("ClinPharm AI Code", WD_STYLE_TYPE.PARAGRAPH)
    ...
    run = footer_p.add_run("ClinPharm AI  •  Public or synthetic examples only  •  ")
    run.font.name = FONT
    run.font.size = Pt(7.2)
...
def markdown_to_docx(source: Path, output: Path) -> None:
    ...
    document.core_properties.author = "ClinPharm AI"
    document.core_properties.subject = "Portable professional and project context"
    document.core_properties.keywords = "ClinPharm AI, pharmaceutical, professional context"
    ...
    document.core_properties.last_modified_by = "ClinPharm AI"
    ...
    for raw in [*lines, ""]:
        ...
        if quote:
            flush_paragraph()
            p = document.add_paragraph(style="ClinPharm AI Quote")
