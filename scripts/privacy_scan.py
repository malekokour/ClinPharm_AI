...
def scan_tree(
    ...
    files = candidates(scan_root, ignored)
    docx_members = 0
    ...
    for path in files:
        ...
        if path.is_symlink():
            findings.append({"path": relative, "rule": "symlink"})
    ...
    print(
        ...
        f"{video_metadata_strings} video metadata strings, and "
        f"{archive_members} archive members; 0 findings"
    )
    return 0
